...
#    st.markdown("<h2 style='text-align: center; color: black;'>This Dashboard can be used to optimise Fly Ash Procurement costs for Ultra Tech Cements. </h2>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: black;'>This Dashboard can be used to optimise Fly Ash Procurement costs for Ultra Tech Cements. The modules available in the dashboard are : </h3>", unsafe_allow_html=True)
...
#    st.write("This is a dashboard for optimising fly ash linkages")
    ...
    if st.sidebar.checkbox("Data Viewer"):
        ...
        #input data
        plant_demand=pd.read_excel('final_input_file1.xlsx','Demand_new')
        source_capacity=pd.read_excel('final_input_file1.xlsx','Capacity_new')
        freight_cost=pd.read_excel('final_input_file1.xlsx','Freight_cost')
        basic_cost=pd.read_excel('final_input_file1.xlsx','Basic_cost')
        key_plant=pd.read_excel('final_input_file1.xlsx','key_plant')
        key_source=pd.read_excel('final_input_file1.xlsx','key_source')
        df_input=pd.read_excel(r'input_file_code_0708_updated.xlsx')
...
#        df_input=df_input["Source Name"].isin(key_source.Name.unique().tolist()) 
        #distance file
        df_dis=pd.read_csv(r'distance_final1.csv')
...
#                st.write(source_capacity)
                ...
                if st.sidebar.checkbox("Run the Optimizer"):
                    ...
                    st.markdown('---')
                    try:
                        source_capacity=pd.read_csv(r'source_capacity_upd.csv')
                    except:
                        source_capacity=source_capacity.copy()
                    try:
                        plant_demand=pd.read_csv('plant_demand_upd.csv')
                    except:
...
#                    st.write(data_map_o)
                    ...
                    if len(fly_ash_type)!=0:
                        ...
                        d_prim4=d_prim4.reset_index(drop=True)
                    ...
                    if len(source_list)!=0:
                        ...
                        data_map1=data_map1[data_map1['source_name_given'].isin(source_list)]
                        data_map1=data_map1.reset_index(drop=True)
                        d_prim4=d_prim4[d_prim4['Source'].isin(source_list)]
                        d_prim4=d_prim4.reset_index(drop=True)
                    ...
                    if len(plant_list)!=0:
                        ...
                        data_map1=data_map1[data_map1['Cluster/Unit'].isin(plant_list)]
                        data_map1=data_map1.reset_index(drop=True)
                        d_prim4=d_prim4[d_prim4['Plant'].isin(plant_list)]
                        d_prim4=d_prim4.reset_index(drop=True)
