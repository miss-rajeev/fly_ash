# -*- coding: utf-8 -*-
"""
Created on Wed Dec  9 10:48:42 2020

@author: aishwarya.rajeev-st
"""
...
#"""
def to_excel(source_capacity,plant_demand,basic_cost,freight_cost):
    output = BytesIO()
...
def get_plant_geo():
    df_dis=pd.read_excel('final_input_file.xlsx','LatLong_name_mapping')
    return df_dis
...
def main():
    ...
    st.sidebar.write(" ")
    st.sidebar.write(" ")
    st.markdown("<h2 style='text-align: left; color: black;'>Welcome to the Dashboard</h2>", unsafe_allow_html=True)
    ...
    if st.sidebar.checkbox("Data Viewer"):
        ...
        #input data
        plant_demand=pd.read_excel('final_input_file.xlsx','Demand_new')
        source_capacity=pd.read_excel('final_input_file.xlsx','Capacity_new')
        freight_cost=pd.read_excel('final_input_file.xlsx','Freight_cost')
        basic_cost=pd.read_excel('final_input_file.xlsx','Basic_cost')
        key_plant=pd.read_excel('final_input_file.xlsx','key_plant')
        key_source=pd.read_excel('final_input_file.xlsx','key_source')
        df_input=pd.read_excel('final_input_file.xlsx','Existing Data')
...
#        df_input=df_input["Source Name"].isin(key_source.Name.unique().tolist()) 
  ...
  #    file_lat_long_groupby=file_lat_long.groupby(['FromID']).count().reset_index()
        ...
        file_view=st.selectbox("Select appropriate option to view data", ["Demand","Capacity","Basic cost","Freight cost"])
        if file_view == "Demand":
        ...
        val = to_excel(source_capacity,plant_demand,basic_cost,freight_cost)
    ...
    #        nl2="Optimal"
            ...
            st.write(data_map2["source_name_given"].nunique())
...
#                    
                    ...
                    #'''
                    #prepare existing cost file
                    #'''
                    #data1=df_input[['source_name_given','Cluster_code','grade_new','Qty(LE)(LMT)','Basic Rate(Rs/MT)','Other(Rs/MT)']]
...
#                    
                    ...
                    def retro_dictify(frame):       #create dictionary
                        ...
                        for row in frame.values:
                            ...
                            here[row[-2]] = row[-1]
                        return d
#                    df_ptpkm_dis=df_ptpkm_dis.fillna(0)
                    
                    #'''
                    #prpepare dictionary
                    #'''
#                    ptpkm_ptpkm_dict=retro_dictify(df_ptpkm_dis[['FromID','ToID','ptpkm']])
#                    ptpkm_km_dict=retro_dictify(df_ptpkm_dis[['FromID','ToID','Kilometres']])
...
#                    plant_demand=plant_demand.rename(columns={'Qty(LE)(LMT)':'Demand(LMT)'})
#                    plant_demand['Demand(LMT)'].sum()
#                    source_capacity=source_capacity.rename(columns={'Qty(LE)(LMT)':'capacity(LMT)'})
#                    source_capacity['capacity(LMT)'].sum()
                    
#                    source_cap_dict=retro_dictify(source_capacity[['grade_new','Source_code','capacity(LMT)']])
#                    pd_dict=retro_dictify(plant_demand[['grade_new','Source_code','Demand(LMT)']])
#                    basic_cost_dict=retro_dictify(basic_cost[['grade_new','Source_code','Basic Rate(Rs/MT)']])
#                    handling_cost_dict=retro_dictify(handling_cost[['grade_new','Source_code','Other(Rs/MT)']])
#                    
#                    df_input2['grade_new']=df_input2['material_group_new']+"="+df_input2['category']
                    #df_input=df_input[~(df_input['Source_code'].isnull())]
                  
                    df_dis=get_plant_geo()
                    df_dis['FromID']=df_dis['FromID'].astype(str)
                ...
                #        if len(df_final) !=  0:
                #            df_final['indicator'] = df_final.apply(assign_indicator, axis=1)
...
#                    st.write(data1)
                    ...
                    data_map_o=pd.merge(data_map_o,file_lat_long_new,how='left',left_on=['Source_key'],right_on=['FromID'])
                    data_map_o=data_map_o.rename(columns={'FromLatitude':'Source_latitude','FromLongitude':'Source_longitude'})
                    del data_map_o['FromID']
                    data_map_o=pd.merge(data_map_o,file_lat_long_new,how='left',left_on=['Plant_key'],right_on=['FromID'])
...
#        
                    ...
                    if len(fly_ash_type)!=0:
                        data1=data1[data1['material_group_new'].isin(fly_ash_type)]
                        data_map1=data_map1[data_map1['material_group_new'].isin(fly_ash_type)]
                        d_prim4=d_prim4[d_prim4['Grade'].isin(fly_ash_type)]
                    ...
                    source_list=st.multiselect("Select sources",d_prim4['Source_key'].unique().tolist(),key=2)
                    if len(source_list)!=0:
                        data1=data1[data1['source_name_given'].isin(source_list)]
                        data_map1=data_map1[data_map1['source_name_given'].isin(source_list)]
                        d_prim4=d_prim4[d_prim4['Source_key'].isin(source_list)]
                        data_map_o=data_map_o[data_map_o['Source_key'].isin(source_list)]
                    else:
                        data1=data1.copy()
                        data_map1=data_map1.copy()
                        d_prim4=d_prim4.copy()
                        data_map_o=data_map_o.copy()
                    plant_list=st.multiselect("Select plant",d_prim4['Plant_key'].unique().tolist(),key=2)
                    if len(plant_list)!=0:
                        data1=data1[data1['Cluster/Unit'].isin(plant_list)]
                        data_map1=data_map1[data_map1['Cluster/Unit'].isin(plant_list)]
                        d_prim4=d_prim4[d_prim4['Plant_key'].isin(plant_list)]
                        data_map_o=data_map_o[data_map_o['Plant_key'].isin(plant_list)]
                    else:
                        data1=data1.copy()
                        data_map1=data_map1.copy()
                        d_prim4=d_prim4.copy()
                    ...
                    if len(category_list)!=0:
                        data1=data1[data1['category'].isin(category_list)]
                        data_map1=data_map1[data_map1['category'].isin(category_list)]
                        d_prim4=d_prim4[d_prim4['Category'].isin(category_list)]
